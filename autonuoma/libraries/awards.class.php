...
class awards {
	...
	public function getContractList($limit, $offset) {
		$query = "  SELECT `apdovanojimas`.`id_Apdovanojimas`,
						   `apdovanojimas`.`pavadinimas` AS `Pav`,
						   `ceremonija`.`pavadinimas`
					FROM `apdovanojimas`
						LEFT JOIN `ceremonija`
							ON `apdovanojimas`.`fk_Ceremonijaid_Ceremonija`=`ceremonija`.`id_Ceremonija`
	...
	public function getContractListCount() {
		$query = "SELECT COUNT(*) as 'kiekis' FROM apdovanojimas";
		$data = mysql::select($query);
